...
export const CardSelectorModal: React.FC<CardSelectorModalProps> = ({
  ...
  const [selectedItemType, setSelectedItemType] = useState<'all' | 'word' | 'sentence' | 'passage'>('all');
  ...
  const filteredItems = useMemo(() => {
    return vocabularyItems.filter((item) => {
      ...
      const effectiveType = item.itemType || (item.category === 'passage' ? 'passage' : item.category === 'sentence' ? 'sentence' : 'word');
      const matchType = selectedItemType === 'all' || effectiveType === selectedItemType;
      ...
      const matchSearch =
        ...
        (item.passageTitle && item.passageTitle.toLowerCase().includes(query)) ||
        (item.banglaTranslit && item.banglaTranslit.toLowerCase().includes(query)) ||
        item.english.toLowerCase().includes(query);

      return matchChapter && matchType && matchSearch;
    });
  }, [searchQuery, selectedChapterId, selectedItemType]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="p-3 sm:p-4 border-b border-stone-200 flex flex-col gap-2.5">
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 text-base sm:text-sm rounded-xl border border-stone-300 focus:outline-hidden focus:ring-2 focus:ring-emerald-500 font-bengali"
              />
            </div>
            <select
              value={selectedChapterId}
              onChange={(e) => setSelectedChapterId(e.target.value)}
              className="px-3 py-2 text-xs sm:text-sm rounded-xl border border-stone-300 bg-white font-bengali focus:outline-hidden focus:ring-2 focus:ring-emerald-500"
            >
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-amber-100 hover:bg-amber-200 text-amber-900 text-xs font-semibold font-bengali flex items-center justify-center gap-1.5 transition border border-amber-200 shrink-0 min-h-[38px]"
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none">
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-semibold font-bengali transition shrink-0 ${
                selectedItemType === 'all'
                  ? 'bg-stone-800 text-white shadow-xs'
                  : 'bg-stone-100 text-stone-600 hover:bg-stone-200'
              }`}
            >
              সকল ({vocabularyItems.length})
            </button>
            <button
              type="button"
            ...
            <button
              type="button"
              onClick={() => setSelectedItemType('sentence')}
              className={`px-3 py-1 rounded-lg text-xs font-semibold font-bengali transition shrink-0 ${
                selectedItemType === 'sentence'
                  ? 'bg-emerald-700 text-white shadow-xs'
                  : 'bg-stone-100 text-stone-600 hover:bg-stone-200'
              }`}
            >
            ...
            <button
              type="button"
              onClick={() => setSelectedItemType('passage')}
              className={`px-3 py-1 rounded-lg text-xs font-semibold font-bengali transition shrink-0 ${
                selectedItemType === 'passage'
                  ? 'bg-amber-700 text-white shadow-xs'
                  : 'bg-amber-100/70 text-amber-900 hover:bg-amber-200'
              }`}
            >
              পাঠের অনুচ্ছেদ (Passages)
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 max-h-[380px] grid grid-cols-2 sm:grid-cols-3 gap-2.5 scrollbar-thin scrollbar-thumb-stone-300">
          {filteredItems.map((item) => {
            ...
            const isPassage = item.itemType === 'passage' || item.category === 'passage';
            const isSentence = item.itemType === 'sentence' || item.category === 'sentence';
            ...
            return (
              <div
                ...
                onClick={() => {
                  ...
                  if (isPassage && selectedMode === 'photo') {
                    setSelectedMode('ar_to_bn');
                  }
                }}
              ...
              >
                ...
                {/* Type Badge */}
                {isPassage && (
                  <span className="self-start px-2 py-0.5 rounded-md bg-amber-100 text-amber-800 text-[10px] font-bold font-bengali mb-1.5">
                    অনুচ্ছেদ
                  </span>
                )}
                {isSentence && (
                  <span className="self-start px-2 py-0.5 rounded-md bg-sky-100 text-sky-800 text-[10px] font-bold font-bengali mb-1.5">
                    বাক্য
                  </span>
                )}
                ...
                {item.passageTitle && (
                  <div className="text-xs font-bold text-amber-900 font-bengali mb-1">
                    {item.passageTitle}
                  </div>
                )}
                <div className="font-arabic text-base sm:text-xl font-bold text-emerald-950 dir-rtl leading-snug line-clamp-2">
                  {item.arabic}
                </div>
                <div className="text-xs font-semibold font-bengali text-stone-800 mt-1 line-clamp-2">
                  {item.bangla}
                </div>
                {item.banglaTranslit && (
                  <div className="text-[10px] text-stone-400 font-bengali truncate w-full">
                    {item.banglaTranslit}
                  </div>
                )}
              </div>
